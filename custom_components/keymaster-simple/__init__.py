...
async def homeassistant_started_listener(
    ...
    locks_to_watch: List[KeymasterSimpleLock],
    evt: Event = None,
):
...
class LockUsercodeUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        ...
        self._primary_lock: KeymasterSimpleLock = hass.data[DOMAIN][config_entry.entry_id][
            PRIMARY_LOCK
        ]
        self._child_locks: List[KeymasterSimpleLock] = hass.data[DOMAIN][
